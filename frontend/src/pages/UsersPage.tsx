...
export function UsersPage() {
  ...
  const save = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['users'] }); setModalOpen(false); setForm(EMPTY) },
  ...
  return (
    <div>
      <PageHeader title="Usuários e perfis" action={<Button onClick={() => { setForm(EMPTY); setError(null); setModalOpen(true) }}>Novo usuário</Button>} />
        ...
        <Table headers={['Nome', 'E-mail', 'Perfil', 'Status', '']}>
          {data?.map((u) => (
            <tr key={u.id} className="hover:bg-gray-50">
              ...
              <td className="px-4 py-2"><Badge color={u.active ? 'green' : 'gray'}>{u.active ? 'Ativo' : 'Inativo'}</Badge></td>
              <td className="px-4 py-2 text-right">
                ...
                <Button variant="ghost" onClick={() => remove.mutate(u.id)}>Desativar</Button>
              </td>
            </tr>
          ))}
        </Table>
