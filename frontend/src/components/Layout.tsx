...
export function Layout() {
  ...
  const ThemeButton = ({ label }: { label?: boolean }) => (
    <button
      ...
      className="flex items-center gap-2 rounded-md px-2 py-1 text-gray-300 hover:bg-white/10 hover:text-white"
    >
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      <aside
        className={cn(
          'flex flex-shrink-0 flex-col bg-gray-900 text-gray-100 transition-all duration-200',
          'fixed inset-y-0 z-40 md:static',
          collapsed ? 'w-16' : 'w-64',
          mobileOpen ? 'left-0' : '-left-64 md:left-0',
        )}
      >
        <div className={cn('flex items-center gap-2 px-3 py-4 text-lg font-bold', collapsed && 'justify-center')}>
          {settings?.logoUrl ? (
            <img src={settings.logoUrl} alt="logo" className="h-8 w-8 flex-shrink-0 rounded" />
          ) : (
            <Building2 className="h-6 w-6 flex-shrink-0" style={{ color: 'var(--color-secondary)' }} />
          )}
        ...
        <button
          ...
          className="hidden items-center justify-center border-t border-white/10 py-2 text-gray-400 hover:bg-white/5 hover:text-white md:flex"
        >
        ...
        <div className={cn('border-t border-white/10 px-3 py-3 text-xs', collapsed && 'flex flex-col items-center gap-2')}>
          {!collapsed && (
            <>
              ...
              <div className="mt-2 flex items-center justify-between">
                <ThemeButton label />
                <button onClick={handleLogout} className="flex items-center gap-1 text-gray-300 hover:text-white">
          ...
          {collapsed && (
            <>
              <ThemeButton />
              <button onClick={handleLogout} title="Sair" className="text-gray-300 hover:text-white">
